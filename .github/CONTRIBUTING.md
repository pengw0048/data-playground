...
- `make preflight` is green (~15s). A fast static gate — artifact hygiene, ruff, basedpyright, the
  OpenAPI contract, and migrations/core-contract — that catches the mechanical CI misses cheaply. It
  intentionally skips the full kernel suite, web, and PostgreSQL; push and let CI run those heavy suites.
