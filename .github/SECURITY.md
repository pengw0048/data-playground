...
| SAST (CodeQL) | `codeql.yml` | Pull requests, weekly schedule, and `workflow_dispatch` — results on the **Code scanning** tab |
| Secret scanning (CI) | `secret-scan.yml` | Pull requests and `workflow_dispatch` — [gitleaks](https://github.com/gitleaks/gitleaks) fails the job on detected secrets |
