...
- ✅ **Arch / extensibility (11/11, from `REVIEW.md`)** — concurrency: a run/preview now runs on its OWN DuckDB cursor (`db.run_scope()`) instead of holding one process-global lock for the whole run, so a long run no longer freezes every other user (empirically validated cursor isolation + per-cursor `interrupt` on DuckDB 1.5.4); a lost/evicted run returns a terminal status instead of a 404 that stranded the node. Collab: a peer's merged edit is cached locally but no longer re-PUT by every client (N-way write-amplification gone), with a `pagehide` flush so the originating editor's in-flight save survives a tab close; node drags commit to the store only on drag-stop (no per-frame O(n) rewrite / socket storm), preserving the live position of a node being dragged. Extensibility: deleted the dead `plugins/base.py` SPI (moved the real contract map into `plugins/__init__.py`), added a typed `NodeLowering` protocol + `min_core_api` plugin version negotiation, aligned the drifted `sql` node spec + a front/back parity test, and dropped the inert `CapabilityProvider.predicate/columns`. Honesty: reconciled the "one kernel per session" docstrings with the real shared multi-user model, and hoisted `main.py`'s ~14 function-local imports to module top + extracted `current_user` into `kernel/security.py`. Verified: 73 kernel pytest + 43 e2e green; changes adversarially re-reviewed (2 collab defects caught and fixed before ship). The full `main.py`→routers file-split was intentionally NOT done — pure reorganization with real un-gating risk and no functional payoff.
