"""Durable dataset-scoped task that applies one certified keyed upsert as a new head.

Revision ID: 0037_keyed_upsert
Revises: 0036_restore_revision
"""
...
revision = "0037_keyed_upsert"
down_revision = "0036_restore_revision"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.create_table(
        "keyed_upsert_task_envelopes",
        sa.Column("task_id", sa.String(), nullable=False),
        sa.Column("base_dataset_id", sa.String(length=128), nullable=False),
        sa.Column("base_revision_id", sa.String(length=256), nullable=False),
        sa.Column("payload_dataset_id", sa.String(length=128), nullable=False),
        sa.Column("payload_revision_id", sa.String(length=256), nullable=False),
        sa.Column("upsert_intent_doc", sa.Text(), nullable=False),
        ...
        sa.UniqueConstraint("write_idempotency_key", name="uq_keyed_upsert_write_key"),
...
def downgrade() -> None:
    if op.get_bind().execute(
            sa.text("SELECT 1 FROM keyed_upsert_task_envelopes LIMIT 1")).first():
        raise RuntimeError("cannot downgrade while keyed upsert tasks are retained")
    op.drop_table("keyed_upsert_task_envelopes")
    with op.batch_alter_table("durable_tasks") as batch:
        batch.drop_constraint("ck_durable_task_kind", type_="check")
        batch.drop_constraint("ck_durable_task_subject", type_="check")
