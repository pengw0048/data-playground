...
the caller's DuckDB connection.  It does not create durable sidecars or resolve catalog heads;
later write paths retain the coverage evidence they need.
"""
...
def freeze_row_identity_spec_from_schema(
    ...
    exact = _exact_ref(dataset_ref)
    declared = _declared_keys(key_columns)
    # A coverage producer must provide a type this V1 contract can prove.
