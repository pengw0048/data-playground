...
- **Static gates:** kernel uses `ruff` + `basedpyright` (the `python-quality` CI job; `make lint`) plus `pytest` with `error::DeprecationWarning:hub.*` treated as errors; web uses `tsc`. `make preflight` runs artifact hygiene, Ruff, basedpyright, the OpenAPI check, and selected graph/SQL/migration tests; it does not run web type-checking or Vitest. There is no black/eslint.
