...
const EVIDENCE_DIR = path.join(REPO_ROOT, 'web', 'test-results', 'acceptance', 'issue-639')
...
test.describe('keyed-upsert release acceptance @acceptance-keyed-upsert', () => {
  ...
  test('certifies the Write Inspector keyed-upsert journey against independently recomputed evidence', async ({ page }) => {
    ...
    try {
      ...
      const control = page.getByTestId('inspector').getByLabel('Keyed upsert setup')
      ...
      await published.getByRole('link', { name: 'Open dataset' }).click()
      ...
      await expect(page.getByTestId('dataset-viewer')).not.toContainText(target.revisionId)
