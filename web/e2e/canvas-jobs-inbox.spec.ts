...
test('a real managed Write retains one bounded cross-surface evidence chain @ux-smoke', async ({ page }) => {
  ...
  try {
    ...
    await expect(appMenu.getByRole('menuitem', { name: 'Inbox', exact: true })).toBeVisible()
    await expect(appMenu.getByRole('menuitem', { name: 'Run history', exact: true })).toHaveCount(0)
    await page.keyboard.press('Escape')
    await expect(appMenu).toBeHidden()
    await page.getByTestId('canvas-menu').click()
    await expect(canvasMenu.getByRole('menuitem', { name: 'Run history', exact: true })).toBeVisible()
    await page.keyboard.press('Escape')
    ...
    await page.getByTestId('app-menu').click()
    await expect(appMenu).toBeVisible()
    await appMenu.getByRole('menuitem', { name: 'Inbox', exact: true }).click()
