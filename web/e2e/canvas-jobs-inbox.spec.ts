...
test('a real managed Write retains one bounded cross-surface evidence chain @ux-smoke', async ({ page }) => {
  ...
  try {
    ...
    await expect(page.getByRole('button', { name: 'Canvas actions' })).toHaveCount(0)
    await page.keyboard.press('Escape')
    await expect(appMenu).toBeHidden()
    await page.getByTestId('canvas-inbox').click()
    const inboxPreview = page.getByRole('dialog', { name: 'Inbox preview' })
    await expect(inboxPreview).toBeVisible()
    await inboxPreview.getByRole('button', { name: 'View all Inbox' }).click()
    ...
    await page.goto(`/#/canvas/${canvasId}`)
    await page.reload()
    const inboxTrigger = page.getByTestId('canvas-inbox')
    const unreadBadge = page.getByTestId('canvas-inbox-unread-badge')
    await expect(unreadBadge).toBeVisible()
    const [triggerBox, badgeBox] = await Promise.all([inboxTrigger.boundingBox(), unreadBadge.boundingBox()])
    ...
    await expect(history.getByText('Source Starter events')).toBeVisible()
    await expect(history).not.toContainText('Dataset ID')
    await expect(history).not.toContainText('Revision ID')
