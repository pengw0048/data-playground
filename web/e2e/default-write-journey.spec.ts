...
test.describe('default fresh-workspace write journey @acceptance-default-journey', () => {
  ...
  test('certifies the default-kernel Source → transform → Write → revision → Jobs/Inbox journey', async ({ page }) => {
    ...
    await expect(reloadedWriteCard).toContainText(`Published · ${outputName}`)
    await expect(reloadedWriteCard).not.toContainText(dataset!.revisionId)
    ...
    await expect(recoveredResult).toContainText('Published')
