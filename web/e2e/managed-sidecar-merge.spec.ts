...
test('uses the installed sidecar fixture through the Write inspector and reopens exact results @ux-smoke', async ({ page }) => {
  ...
  try {
    ...
    await expect(inspector.getByLabel('Managed sidecar check')).toContainText('Ready to merge saved sidecar')
    ...
    const baseLink = reopened.getByRole('link', { name: 'Open saved base' })
