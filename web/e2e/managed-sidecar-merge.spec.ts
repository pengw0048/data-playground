...
test('uses the installed sidecar fixture through the Write inspector and reopens exact results @ux-smoke', async ({ page }) => {
  ...
  try {
    ...
    await expect(inspector.getByLabel('Column merge setup')).toBeVisible()
    ...
    await inspector.getByLabel('Saved dataset merge mode 1').selectOption('replace')
    await inspector.getByLabel('Destination column 1').fill('replace_me')
    await inspector.getByRole('button', { name: 'Check setup' }).click()
    await expect(inspector.getByLabel('Column merge check')).toContainText('Ready to merge')
    ...
    await inspector.getByRole('button', { name: 'Start merge' }).click()
    const task = await json<{ taskId: string }>(await submitted, 'submit browser managed sidecar merge')
    await expect(inspector.getByRole('link', { name: 'Open merged dataset' })).toBeVisible({ timeout: 20_000 })
    ...
    const childLink = reopened.getByRole('link', { name: 'Open merged dataset' })
    const baseLink = reopened.getByRole('link', { name: 'Open original dataset' })
    ...
    if (!childHref || !baseHref) throw new Error('saved version links omitted their routes')
    ...
    await expect(page.getByTestId('dataset-viewer')).not.toContainText(child.revisionId)
    ...
    await expect(page.getByTestId('dataset-viewer')).not.toContainText(base.revisionId)
  } finally {
