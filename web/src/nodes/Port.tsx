...
export function Port({ spec, side, index, count, nodeId }: {
  ...
  const base: React.CSSProperties = {
    width: addAffordance ? 15 : 11,
    height: addAffordance ? 15 : 11,
    ...
    background: emphasized ? tok.color : 'hsl(var(--card))',
    border: `1.5px solid ${tok.color}`,
    top,
    [isSource ? 'right' : 'left']: addAffordance ? -8 : -6,
  ...
  return (
    ...
    >
      {/* the "+" add-affordance on an output port (counter-rotated for diamonds) */}
      {addAffordance && (
