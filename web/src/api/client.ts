...
export const api = {
  ...
  plan: (doc: CanvasDoc, targetNodeId: string) =>
    req<{ regions: { id: string; outputNode: string; backend: string; worker: string | null; nodeIds: string[]; tier: string | null; rows: number | null; confidence: string; requires?: string; unsatisfied?: boolean; available?: string }[]; error?: string }>(
