...
export function ManagedSidecarMergeControl({ nodeId, compact = false }: { nodeId: string; compact?: boolean }) {
  ...
  const draftError = useMemo(() => {
    ...
    if (suggestedFields) {
      for (const identity of identities) {
        ...
        if (!base || !source) return `Identity “${identity}” is not present in both saved-version schemas.`
  ...
  if (!enabled) return <div className="mt-2 rounded-md border border-border bg-muted/20 p-2 text-[10.5px] text-muted-foreground">
    <div className="font-semibold text-foreground">Merge a saved sidecar version</div>
    <div className="mt-0.5">Use a direct saved Source version as the sidecar, then choose the destination's current version and the column mappings.</div>
  ...
  return <div aria-label="Managed sidecar column merge" className={compact ? 'mt-2 text-[10.5px]' : 'mt-3 rounded-md border border-border bg-muted/30 p-2'}>
    {!compact && <><div className="font-semibold text-[11px] text-foreground">Add or replace columns in an existing dataset</div>
      <div className="mt-0.5 text-[10.5px] leading-snug text-muted-foreground">The upstream transform already published this saved sidecar version. This action merges only the mapped columns into a local dataset; it never writes back to an external provider.</div></>}
    {!sidecar && <div role="alert" className="mt-2 text-[10.5px] text-destructive">Connect one saved local Source version directly to this Write before configuring the merge.</div>}
    {sidecar && <div className="mt-2 rounded border border-border bg-background p-2 text-[10px] text-muted-foreground"><strong className="text-foreground">Saved sidecar version</strong><div className="mt-0.5 break-all font-mono">{sidecar.datasetId}@{sidecar.revisionId}</div></div>}
    ...
    {selectedBase && <div className="mt-2 rounded border border-border bg-background p-2 text-[10px] text-muted-foreground"><strong className="text-foreground">Saved destination version</strong><div className="mt-0.5 break-all font-mono">{selectedBase.datasetId}@{selectedBase.revisionId}</div></div>}
    ...
    {identityCandidates.length > 0 && !compact && <div className="mt-1 flex flex-wrap items-center gap-1"><span className="text-[10px] text-muted-foreground">Suggested identity columns:</span>{identityCandidates.map((candidate) => <Button key={candidate.join('\u0000')} size="sm" variant="outline" className="h-6 px-2 text-[10px]" onClick={() => useSuggestion(candidate)} disabled={!canEdit || intentLocked}>Use {candidate.join(', ')}</Button>)}<span className="text-[10px] text-muted-foreground">Run the check below to confirm the setup.</span></div>}
