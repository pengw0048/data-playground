...
export function KernelBadge({ kernelUp, kernelInfo }: { kernelUp: boolean; kernelInfo: KernelInfo | null }) {
  ...
  const restart = async () => {
    ...
    try {
      ...
      pushToast(r.restarted ? 'Canvas worker restarting…' : 'No worker running — one starts on the next run', 'success')
      await refresh()
    } catch (e) {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          type="button"
          aria-label="Canvas worker status"
      ...
      <PopoverContent align="end" className="w-72 p-3">
        <div className="mb-2 text-xs font-semibold text-foreground">Canvas worker</div>
        <div className="flex flex-col gap-2 text-[11.5px]">
          ...
          {cat === 'offline' && <p className="text-muted-foreground">Worker unreachable — showing last-known state.</p>}
          <Button
            variant="outline"
            size="sm"
            className="mt-1 w-full"
            disabled={!canEdit || !kernelUp || restarting}
            title={!canEdit ? 'View-only canvas' : !kernelUp ? 'Hub offline' : 'Restart this Canvas\u2019s worker'}
            onClick={restart}
          >
            <Icon name="refresh" size={13} /> {restarting ? 'Restarting…' : 'Restart worker'}
          </Button>
        </div>
      </PopoverContent>
    </Popover>
