...
- 🟡 能力查看器标签页（§1.2）· Agent dock 需配模型（§1.6）· 运行取消纯 Python 循环（§2.3）· 成本门控（§2.3）· 独立 loop 节点（§2.4）· Arrow/Feather 扫描（§3.1）· 原子覆盖写-对象存储（§3.2）· 对象存储浏览（§3.4）· 实时协作 sticky-routing（§4.1）· Capabilities 插件扩展（§5.6）· 导入器 501（§5.7）· pod/Ray 内置插件（§2.3，参考 pool 后端已内置）· 分布式/解耦执行（§6.6，per-canvas kernel 已解耦 web 层，参考 pod 底座已内置 `DP_KERNEL_SPAWNER=pod`、真集群未验）· shadcn 迁移（§6.4）
...
### §2.3 执行器（ExecutionBackend）· 11✅ 3🟡 · `↗ per-user 选择见 §6.2`
...
- ✅ **ExecutionBackend SPI（add_runner；替代 runner）** — runtime_checkable 协议；内置 Local + Subprocess；pick_runner 按名字选、run_index 路由。 `kernel/hub/backends.py:39-57; deps.py:211 pick_runner, 181 run_index`
