...
- ✅ **ER / UML 视图（Relationships 视图）** — React Flow 实体图：每数据集一实体（列 + 🔑 主键徽章），声明关系实线带基数、命名候选虚线；点列声明主键、拖两表声明 join（键+基数实测）、点边删除。 `web/src/views/ERDiagram.tsx; web/src/views/Shell.tsx`
