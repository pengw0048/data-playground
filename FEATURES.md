...
### §2.4 计算节点执行 · 5✅ 1🟡
- ✅ **chart 图表节点（可视化）** — lower 成 (x,y) 序列：bar/line/area 走 `agg(y) group by x`（预览也跑全量、诚实，上游有 Python transform 则拒；TRY_CAST 让非数值 min/max 降级为空而非报错），scatter 走原始点；数据面板渲染零依赖、明暗自适应 SVG；输出可继续接。 `kernel/kernel/executors/engine.py chart 分支; web/src/panels/DataPanel.tsx ChartView; web/src/nodes/kinds/chart.tsx`
