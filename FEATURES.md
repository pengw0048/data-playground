...
### §3.3 目录（Catalog）· 规模化到数千张表 · 17✅ · `↗ SPI 契约见 §5.5`
- ✅ **服务端分页浏览（filter + sort + paginate，下推到 DB）** — `GET /catalog/tables` 现接受 `q / folder / tags / owner / hasColumns / uris / sort / order / limit / offset`，把过滤+排序+分页**下推成一条带索引的 SQL**（`catalog_entries` 促列 + `catalog_tags/catalog_columns` join 表 + EXISTS 子查询），内存/带宽只随 `limit` 走、不随目录规模走——这是替掉"读时把整个目录拉进内存再过滤"（老模型撑不过几百张表）的核心。响应体仍是裸 `list[CatalogTable]`（向后兼容），总数/是否有下一页走 `X-Total-Count` / `X-Has-More` 响应头。 `kernel/hub/metadb.py catalog_query; kernel/hub/plugins/catalog.py list_page; kernel/hub/routers/catalog.py list_tables`
- ✅ **分面（facets：folder/tag/owner + 计数）** — `GET /catalog/facets` 在**当前过滤集**上按维度聚合出候选值+计数（drill-down 语义，与列表同一套过滤），点亮前端 facet rail 的一键可点过滤。 `kernel/hub/metadb.py catalog_facets; kernel/hub/routers/catalog.py catalog_facets`
- ✅ **文件夹/命名空间浏览树（懒展开）** — `folder` 是分隔符路径（`prod/images/curated`）的浏览层级；`GET /catalog/tree?prefix=` 返回某层直接子文件夹（带子树计数）+ 该层直属表（有界样本，`totalTables`/`truncated` 标注是否还有更多），前端按需展开任意规模的树。 `kernel/hub/metadb.py catalog_tree; kernel/hub/plugins/catalog.py browse`
- ✅ **组织原语（folder / tags / owner / description）+ 策展** — `CatalogTable` 新增这些通用、owner-asserted 字段；`PUT /catalog/tables/{id}/metadata` 归档到文件夹、打标签、设 owner/描述（**只改 body 里出现的字段**——缺席=保留，显式 null=清除 owner/描述；不动探测出的 schema）。全通用、不绑定任何外部系统，但与成熟目录的 namespace/tag/owner 模型一一对应，便于外部 provider 往返。 `kernel/hub/metadb.py catalog_set_metadata; kernel/hub/routers/catalog.py set_table_metadata`
- ✅ **search（lexical / semantic / hybrid）** — `GET /catalog/search?q=&mode=`：lexical 按**空白分词、逐词 AND**（每个词命中 name/uri/folder/description/tag/column 任一即算，`curated images` 能找到 `demo/images/curated`；`%`/`_` 按字面匹配不作通配）；semantic 走 embedding 余弦（仅当插件经 `reg.add_embedder` 注册了 embedder 时激活，`facets.semanticAvailable` 广播给 UI，搜索框出现 text|meaning 切换）；hybrid = 两者 RRF 融合。无 embedder 时优雅回退 lexical——离线永远可搜。语义打分矩阵进程内缓存（写失效 + 30s TTL），后台重建索引分页走完全量目录、分批调 embedder。 `kernel/hub/plugins/catalog.py search/semantic_search/_reindex_embeddings; kernel/hub/routers/catalog.py catalog_search` `↗ embedder 接缝见 §5.6`
- ✅ **popularity（usage 计数）排序** — 采样/被下游读取时 best-effort 累加 `usage`，支持"最常用"排序（Amundsen 式发现信号）。 `kernel/hub/metadb.py catalog_bump_usage; routers/catalog.py data_sample; catalog.py register_output`
- ✅ **有界血缘（depth + max_nodes + truncated，含列级边）+ 边批量导出** — `lineage(uri, depth, max_nodes)` 从 DB 一层层 BFS 扩展连通分量并**按深度/节点数封顶**（单层 frontier 抓边也有上限，高扇出枢纽不会一口气拉全表；`truncated` 告知被截断）；边表带 `column` 支持列级溯源。以 uri 为跨系统主键；`GET /catalog/edges?limit&offset`（X-Total-Count）供外部血缘系统批量同步。 `kernel/hub/plugins/catalog.py lineage; kernel/hub/metadb.py catalog_edges_touching/catalog_edges_page; routers/catalog.py lineage/lineage_edges`
- ✅ **O(1) get / resolve（不再全量重合）** — `get_table` / `resolve_ref` 走按 uri/tbl_id/name 的**单条索引查询**，不再"读时把整个目录拉进内存再查找"；跨实例一致由 DB 权威保证。 `kernel/hub/metadb.py catalog_get; kernel/hub/plugins/catalog.py get_table/resolve_ref`
- ✅ **启动时从本地数据目录播种目录** — 发现 parquet/csv/tsv/json/arrow + .lance 目录。 `kernel/hub/plugins/catalog.py _seed`
- ✅ **目录注册（跨重启持久化 + 可选 folder/tags）** — adapter.schema 校验可读，`register_output`/`POST /catalog/register` 写穿 `catalog_entries`（并把 folder/tags/owner/description/row_count/列名镜像到促列+join 表）。 `kernel/hub/routers/catalog.py catalog_register`
...
- ✅ **DB 支撑/跨实例（catalog_entries 促列 + tags/columns/embeddings join 表 + edges）** — 写穿 DB 权威，读走带索引的下推查询（不再每次全量重合）；跨实例一致 + 扛重启。迁移 `0017_catalog_org`（含从旧 doc 回填促列/tag/column 行；回填裸 SQL 对保留字 `"column"` 加引号——Postgres 上不炸；`row_count` 用 BigInteger；排序键 row_count/usage/updated_at 也建索引；有回填测试）。 `↗ 部署见 §6.6` `kernel/hub/metadb.py; kernel/hub/migrations/versions/0017_catalog_org.py`
- ✅ **缺失检测（missing 标记，不静默隐藏）** — overlay 时对本地路径条目探活（只对返回页做，不全表 stat），源文件消失标 `missing=true`（跳过 object-store 与 mem://，不误报）。 `kernel/hub/plugins/catalog.py _overlay`
- ✅ **注销条目（DELETE /catalog/tables/{id}，级联清干净）** — 删条目 + 其 tag/column/embedding 镜像行 **+ 血缘边（双向）+ 声明键 + 声明关系**（不删底层数据）——被删的表不再以幽灵节点残留在血缘/ER 里，同 uri 重新注册也不会继承旧声明键/旧父边；详情抽屉有 Unregister 按钮（missing 条目尤其需要）；批量前缀删 `catalog_delete_prefix` + `dataplay seed-catalog --remove --prefix X`（demo/scale 清理）。 `kernel/hub/plugins/catalog.py unregister; kernel/hub/metadb.py catalog_delete_entry/_delete_catalog_children; kernel/hub/cli.py`
- ✅ **按名字/id 解析 source（图内引用改写）** — `resolve_ref` 把 source 节点里写的数据集名字/catalog id 解析成真实 uri（单条索引查询）；compile/preview/schema/estimate/run 各端点在编译前 `resolve_source_refs` 就地改写，让图可移植。 `kernel/hub/plugins/catalog.py resolve_ref; kernel/hub/graph.py resolve_source_refs`
- ✅ **前端：三栏 Tables 视图 + 虚拟化 + 有界 ER** — 左 folder 树 / 中虚拟化列表（服务端分页 + 无限滚动——换查询滚回顶部、失败可重试、追加页按 id 去重 + debounce 搜索 + 排序 + 有 embedder 时 text|meaning 搜索切换，`web/src/ui/VirtualList.tsx`）/ 右 facet rail；首页加载失败给内联错误 + Retry；行→详情抽屉（列<点击即按列过滤> + 数据预览<懒加载 sample> + 血缘<父/子可点击跳转> + folder/tags/owner/description 策展 + Unregister；role=dialog + Esc 关闭 + 行键盘可达）；source picker 服务端搜索 + 冷启动拉一页 usage 排序的目录页（空目录/无 recents 文案区分）；裸名/`tableId` 引用的 source 也能解析（逐个 `api.table` 回退）；ER 图按 folder 有界渲染（不再全量 O(n²)）；画布只按需拉取其 source 引用的表（工作集，非全量）。 `web/src/views/CatalogView.tsx; web/src/ui/VirtualList.tsx; web/src/nodes/kinds/source.tsx; web/src/views/ERDiagram.tsx; web/src/store/graph.ts ensureCanvasTables`
- ✅ **演示/验收：seed-catalog CLI（含 --remove）** — `dataplay seed-catalog --count N` 注册 N 个合成数据集（跨 folder/tag/owner 空间），一把看清规模化 UX（已验 5000 张表）；`--remove --prefix X` 一把清干净。 `kernel/hub/cli.py _run_seed_catalog; kernel/hub/metadb.py catalog_bulk_seed`
...
- ✅ **Node authoring SDK (`add_node` + `ctx` SQL/Arrow/Polars builders)** — `ctx.sql` exposes its relation as the validated query-scope CTE `input`; `add_node` rejects built-in/registered kind shadowing. `kernel/hub/sdk.py:38-70`
...
### §5.5 目录 SPI · 3✅ · `↗ 内置目录见 §3.3`
- ✅ **目录提供者 SPI（set_catalog，旧协议兼容垫片）** — 正式 `CatalogProvider` runtime_checkable 协议（backends.py；含 `list_page/facets/browse/search/search_modes/get_table/lineage/resolve_ref/set_metadata/register…`；`get_table` 未命中必须抛 `KeyError`；`register_output` 签名与调用点一致——version 可选 + 组织字段 kwargs）；按**旧**协议写的 provider（只有 list_tables/get_table…）经 `reg.set_catalog` 自动包 `CatalogCompat` 垫片——用有界 list_tables 合成 list_page/facets/browse/search（旧 provider 自己的成本模型），新路由不 500；只读的外部目录可子类化 `InMemoryCatalog` 只覆盖读方法、继承 browse/search/lineage/策展机器。 `kernel/hub/backends.py CatalogProvider; kernel/hub/plugins/catalog.py CatalogCompat; deps.py set_catalog`
- ✅ **默认目录=第一方"第一个插件"（自我 dogfood 接缝）** — 内置的 DB 后端 `InMemoryCatalog` **不再由核心特权实例化**，而是由随包发布的第一方插件 `hub/plugins/default_catalog.py` 经 `reg.set_catalog(...)` 注册，且在任何工作区/entry-point 插件**之前**首位加载。所以内置目录是"经公开接缝的第一个实现"而非特权核心路径，之后加载的插件仍可再 `set_catalog` 覆盖（远端元数据服务、SQL 目录…）；无人覆盖时默认胜出，`clone 即用`不变。出现在 `/api/plugins`（source=builtin）。 `kernel/hub/plugins/default_catalog.py; kernel/hub/deps.py _load_bundled`
- ✅ **参考目录插件（SQL/Postgres 支撑）** — 真插件走 set_catalog 接缝：`SqlCatalog(InMemoryCatalog)` 覆盖全部读面（list_tables/get_table/**list_page/facets/browse/search**，sync-then-delegate + 2s 节流），从 SQL `datasets(name, uri)` 表 `_sync`（SQLAlchemy，用公开 register_output），其余继承；`DP_SQL_CATALOG_URL`/`DP_SQL_CATALOG_TABLE` 配置；证明「只读外部目录子类化内置」的路径。 `examples/plugins/dp_sql_catalog; kernel/hub/tests/test_kernel.py:1838`

### §5.6 能力与处理器 · 4✅
- ✅ **add_embedder 接缝（语义目录搜索的可插拔 embedder）** — `reg.add_embedder(fn, model)` 注册一个 `fn(list[str]) -> list[list[float]]`，为目录的 semantic/hybrid 搜索供能（对 name/description/columns 做 embedding）。核心**不内置**任何 embedding 模型（重且有主张的依赖）——由插件提供（参考 `dp_semantic_catalog`，`sentence-transformers` 可选 extra、懒加载、无则回退 lexical）；目录后台 best-effort 重建索引；不支持 embedding 的 provider 直接忽略。 `kernel/hub/deps.py Registry.add_embedder; kernel/hub/plugins/catalog.py set_embedder/semantic_search; examples/plugins/dp_semantic_catalog`
