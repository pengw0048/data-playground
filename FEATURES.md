...
_最后更新：2026-07-06。**150 项功能**（含验收批次新增：chart 图表节点、按名字解析 source、目录缺失检测/注销、DuckDB 原生文件系统沙箱、admin 角色、弱密钥拒绝、跨站 WS 门、启动安全护栏）—— ✅ 134 · 🟡 13 · ⬜ 3。_
...
- ✅ **核外执行（DuckDB 流式 + 溢出磁盘）** — 关系算子原生流式/溢出；temp_directory 显式设为 DP_SPILL_DIR（运维可控）、DP_MEMORY_LIMIT 可限内存；比内存大的数据集在有限内存上限下排序而非 OOM；Python transform 溢出 Parquet 再读回，runner GC。 `kernel/kernel/db.py _apply_session; engine.py:364`

### §2.2 预览与 schema · 3✅
- ✅ **忠实的样本预览（join/sort/vector 对完整输入运行）** — 这些算子预览时以未采样子引擎构建，LIMIT 成诚实 top-N；预览预算 2000 行。 `kernel/kernel/executors/engine.py:118`
...
### §2.4 计算节点执行 · 5✅ 1🟡
- ✅ **chart 图表节点（可视化）** — build 成 (x,y) 序列：bar/line/area 走 `agg(y) group by x`（预览也跑全量、诚实，上游有 Python transform 则拒；TRY_CAST 让非数值 min/max 降级为空而非报错），scatter 走原始点；数据面板渲染零依赖、明暗自适应 SVG；输出可继续接。 `kernel/kernel/executors/engine.py chart 分支; web/src/panels/DataPanel.tsx ChartView; web/src/nodes/kinds/chart.tsx`
...
- ✅ **目录注册（跨重启持久化）** — adapter.schema 校验可读，写穿 catalog_entries（DB 单行/条目），不再靠 settings blob 重注册（重启后从 DB 合并回来）。 `kernel/kernel/routers/catalog.py:75`
...
- ✅ **缺失检测（missing 标记，不静默隐藏）** — overlay 时对本地路径条目探活，源文件消失标 `missing=true`（跳过 object-store 与 mem://，不误报），前端可如实呈现而非假装还在。 `kernel/kernel/plugins/catalog.py _overlay`
- ✅ **注销条目（DELETE /catalog/tables/{id}）** — 锁内删内存态 + DB 行（不删底层数据），清掉陈旧/缺失条目。 `kernel/kernel/routers/catalog.py; kernel/kernel/plugins/catalog.py unregister`
- ✅ **按名字/id 解析 source（图内引用改写）** — `resolve_ref` 把 source 节点里写的数据集名字/catalog id 解析成真实 uri；compile/preview/schema/estimate/run 各端点在编译前 `resolve_source_refs` 就地改写，让图可移植（不必硬编码绝对路径）。 `kernel/kernel/plugins/catalog.py resolve_ref; kernel/kernel/graph.py resolve_source_refs; kernel/kernel/routers/runs.py`
...
- ✅ **本地路径的数据集 URI 限定** — 认证模式下本地路径（register/sample/source 构建）必须在允许 root 内，越界 403；对象存储 URI 不受影响，开放单用户模式不限定。 `kernel/kernel/paths.py:24`
- ✅ **DuckDB 原生文件系统沙箱（统一覆盖含裸 sql）** — 认证 + 无对象存储时，运行连接设 `allowed_directories` + `enable_external_access=false`，让 `read_csv`/`COPY`/裸 `sql` 也逃不出允许 root（不止 source 节点这一层，防绕过）；`enable_external_access` 是进程级一次性单向开关，故在基连接上应用一次，配了对象存储需网络访问则与沙箱互斥。 `kernel/kernel/db.py:110 _apply_session`
...
- ✅ **拒绝弱/已知 DP_AUTH_SECRET（启动即失败）** — 启动时校验密钥不是出厂/示例值，弱密钥会让签名会话可伪造，故 fail fast 而非静默运行。 `kernel/kernel/auth.py:39 reject_weak_secret; main.py:55`
...
- ✅ **管理员角色（is_admin，门控实例级操作）** — 引导/唯一用户即 admin（升级过的库自动补一个 admin，不至无人可管）；建用户 + 实例级设置（对象存储凭证 / agent key / destinations）需 admin，认证模式下非 admin 403。 `kernel/kernel/metadb.py:239 is_admin (migration 0010); routers/workspace.py:87 _require_admin`
...
- ✅ **跨站 WebSocket 同源门（CSWSH 防护）** — 两个 WS 端点先查 Origin 同源，跨站直接拒，堵住浏览器带 cookie 的跨站 WS 劫持。 `kernel/kernel/main.py:62 _cross_site_ws`
...
- ✅ **NodeBuilder 协议（插件构建契约）** — runtime_checkable，单输出 Relation / 多输出 {port:Relation}。 `↗ 引擎侧见 §2.1` `kernel/kernel/backends.py:23-36`
...
### §6.1 应用与服务 · 5✅
- ✅ **dataplay 一条命令启动器 / CLI** — --host/port/workspace/data-dir/no-open/no-seed；播种、配 workspace、uvicorn、开浏览器；workspace 环境变量在 import settings 前导出，故 --workspace 真隔离元数据库/目录/限定 root。 `kernel/kernel/cli.py:15-56`
- ✅ **启动安全护栏（拒非环回免认证绑定 + 日志）** — 免认证下绑非 127.0.0.1 直接拒（除非 DP_AUTH_SECRET 或 DP_ALLOW_INSECURE_BIND=1），否则等于全网任意代码/文件访问；默认输出日志（不再静默），级别走 DP_LOG_LEVEL（归一化非法值不崩 uvicorn）。 `kernel/kernel/cli.py:36-67`
