...
_最后更新：2026-07-06（在这次由清单驱动的清理之后 —— 连线的删除/重连、诚实的 /pipelines/import、移除死掉的错误门控、文档修正）。图例：✅ 已实现 · 🟡 部分实现（见备注） · ⬜ 未实现（仅有脚手架/规划中，或有意省略）。_

**142 项功能 —— ✅ 116 项已实现 · 🟡 22 项部分实现 · ⬜ 4 项未实现。**
...
- ⬜ **branch / loop / variable 控制流节点**（画布与节点交互）—— 强调色以及 'control' 工具栏分类（Toolbar.tsx:12，'Control flow'）都存在，但在任何地方（前端或 kernel/nodespecs.py）都没有节点类型注册为 'control' 分类。工具栏只会保留有 spec 的分类（Toolbar.tsx:27），所以 Control-flow 分组根本不会显示。循环/分支改为通过 'section' 容器的驱动脚本来表达（section.tsx:102）。`web/src/theme/tokens.ts:54`
...
- ⬜ **本地路径的数据集 URI 限定**（数据、适配器与目录）—— 缺口：/catalog/register 和 source 节点接受任意绝对本地路径（abspath+expanduser，没有 root 白名单）；引擎会直接扫描它（engine.py:158）。只有 destinations 浏览这个入口受 root 限定 —— 数据集 READ 并不受限，所以一个已认证的用户可以读取任意本地文件。http(s) 上的 SSRF 已被阻止，但缺少本地文件系统的限定。`kernel/kernel/routers/catalog.py:79`
...
- 🟡 **部署文档**（平台、设计系统与运维）—— README 覆盖了无状态 web 模型、DP_DATABASE_URL→Postgres、对象存储，以及一个用于协作的 nginx 一致性哈希示例。没有专门的部署指南，没有 Dockerfile/compose，也没有反向代理/TLS/systemd 的运维手册。`README.md:77-99 ('Scaling out'); no docs/DEPLOY*.md (docs/ has PRD, TESTING, CONTROL_FLOW only)`
...
- ⬜ **branch / loop / variable 控制流节点** —— 强调色以及 'control' 工具栏分类（Toolbar.tsx:12，'Control flow'）都存在，但在任何地方（前端或 kernel/nodespecs.py）都没有节点类型注册为 'control' 分类。工具栏只会保留有 spec 的分类（Toolbar.tsx:27），所以 Control-flow 分组根本不会显示。循环/分支改为通过 'section' 容器的驱动脚本来表达（section.tsx:102）。 `web/src/theme/tokens.ts:54`
...
- ⬜ **本地路径的数据集 URI 限定** —— 缺口：/catalog/register 和 source 节点接受任意绝对本地路径（abspath+expanduser，没有 root 白名单）；引擎会直接扫描它（engine.py:158）。只有 destinations 浏览这个入口受 root 限定 —— 数据集 READ 并不受限，所以一个已认证的用户可以读取任意本地文件。http(s) 上的 SSRF 已被阻止，但缺少本地文件系统的限定。 `kernel/kernel/routers/catalog.py:79`
...
- 🟡 **部署文档** —— README 覆盖了无状态 web 模型、DP_DATABASE_URL→Postgres、对象存储，以及一个用于协作的 nginx 一致性哈希示例。没有专门的部署指南，没有 Dockerfile/compose，也没有反向代理/TLS/systemd 的运维手册。 `README.md:77-99 ('Scaling out'); no docs/DEPLOY*.md (docs/ has PRD, TESTING, CONTROL_FLOW only)`
