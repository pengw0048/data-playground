...
- ✅ **文件夹/命名空间浏览树（懒展开）** — `folder` 是分隔符路径（`prod/images/curated`）的浏览层级；`GET /catalog/tree?prefix=` 返回某层直接子文件夹（带子树计数）+ 该层直属表，前端按需展开任意规模的树。 `kernel/hub/metadb.py catalog_tree; kernel/hub/plugins/catalog.py browse`
- ✅ **组织原语（folder / tags / owner / description）+ 策展** — `CatalogTable` 新增这些通用、owner-asserted 字段；`PUT /catalog/tables/{id}/metadata` 归档到文件夹、打标签、设 owner/描述（只改传入字段，不动探测出的 schema）。全通用、不绑定任何外部系统，但与成熟目录的 namespace/tag/owner 模型一一对应，便于外部 provider 往返。 `kernel/hub/metadb.py catalog_set_metadata; kernel/hub/routers/catalog.py set_table_metadata`
- ✅ **search（lexical / semantic / hybrid）** — `GET /catalog/search?q=&mode=`：lexical 走 name/folder/tag/column 子串；semantic 走 embedding 余弦（仅当插件经 `reg.add_embedder` 注册了 embedder 时激活）；hybrid = 两者 RRF 融合。无 embedder 时优雅回退 lexical——离线永远可搜。 `kernel/hub/plugins/catalog.py search/semantic_search; kernel/hub/routers/catalog.py catalog_search` `↗ embedder 接缝见 §5.6`
...
- ✅ **有界血缘（depth + max_nodes + truncated，含列级边）** — `lineage(uri, depth, max_nodes)` 从 DB 一层层 BFS 扩展连通分量并**按深度/节点数封顶**（大血缘图不炸 payload，`truncated` 告知被截断）；边表带 `column` 支持列级溯源。以 uri 为跨系统主键。 `kernel/hub/plugins/catalog.py lineage; kernel/hub/metadb.py catalog_edges_touching; routers/catalog.py lineage`
...
- ✅ **DB 支撑/跨实例（catalog_entries 促列 + tags/columns/embeddings join 表 + edges）** — 写穿 DB 权威，读走带索引的下推查询（不再每次全量重合）；跨实例一致 + 扛重启。迁移 `0017_catalog_org`（含从旧 doc 回填促列/tag/column 行）。 `↗ 部署见 §6.6` `kernel/hub/metadb.py; kernel/hub/migrations/versions/0017_catalog_org.py`
...
- ✅ **注销条目（DELETE /catalog/tables/{id}）** — 删条目 + 其 tag/column/embedding 镜像行（不删底层数据）；批量前缀删 `catalog_delete_prefix`（demo/scale 清理）。 `kernel/hub/plugins/catalog.py unregister; kernel/hub/metadb.py catalog_delete_entry/catalog_delete_prefix`
...
- ✅ **前端：三栏 Tables 视图 + 虚拟化 + 有界 ER** — 左 folder 树 / 中虚拟化列表（服务端分页 + 无限滚动 + debounce 搜索 + 排序，`web/src/ui/VirtualList.tsx`）/ 右 facet rail；行→详情抽屉（列 + 血缘 + folder/tags/owner/description 策展）；source picker 改服务端搜索；ER 图按 folder 有界渲染（不再全量 O(n²)）；画布只按需拉取其 source 引用的表（工作集，非全量）。 `web/src/views/CatalogView.tsx; web/src/nodes/kinds/source.tsx; web/src/views/ERDiagram.tsx; web/src/store/graph.ts ensureCanvasTables`
- ✅ **演示/验收：seed-catalog CLI** — `dataplay seed-catalog --count N` 注册 N 个合成数据集（跨 folder/tag/owner 空间），一把看清规模化 UX（已验 5000 张表）。 `kernel/hub/cli.py _run_seed_catalog; kernel/hub/metadb.py catalog_bulk_seed`
...
- ✅ **目录提供者 SPI（set_catalog）** — 正式 `CatalogProvider` runtime_checkable 协议（backends.py；含 `list_page/facets/browse/search/get_table/lineage/resolve_ref/set_metadata/register…`；`get_table` 未命中必须抛 `KeyError`）；只读的外部目录可子类化 `InMemoryCatalog` 只覆盖读方法、继承 browse/search/lineage/策展机器。 `kernel/hub/backends.py CatalogProvider`
