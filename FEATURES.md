...
_最后更新：2026-07-07。**151 项功能**（含验收批次：chart 节点、按名字解析 source、目录缺失检测/注销、DuckDB 沙箱、admin 角色、弱密钥拒绝、跨站 WS 门、启动护栏；以及 Jupyter 式 per-canvas 执行 kernel——**现为默认执行模型**：run/preview/profile 全走 kernel、扛住 hub 重启 + 重连、预览热缓存；`kernel/` 包已改名 `hub/`，仅剩 pod 跨机底座待做））—— ✅ 135 · 🟡 14 · ⬜ 2。_
...
- 🟡 **分布式 / 解耦执行** — **默认**每画布常驻 kernel（见 §2.3 KernelBackend）已让 web/hub 层可重启/重部署而不打断在飞 run（run 在脱离的 kernel 进程跑完 + active-runs 重连），run/preview/profile 全走 kernel + 预览热缓存；心跳门控 reaper（run_states.kernel_id + kernels 心跳）取代了"单实例假设"的旧 reconcile。仅剩 **pod 底座真跨机（P3）** 待做（单机 hub 无法 SIGKILL 别的 host 上的 kernel；pod 用 Service DNS + k8s liveness 解决）。 `kernel/hub/kernel_backend.py; metadb.py reap_orphaned_runs`
