...
- 🟡 **Capabilities 查看器标签页** — 检测 + 标注可用（含插件检测器）+ Media 网格；剩余缺口：查看器标签页仍是单独的前端注册（插件加不了新标签页），vector 标签页已移除只剩内联 chip。 `↗ 前端标签页见 §1.2` `kernel/hub/plugins/capabilities.py; capabilities.tsx:42-50`
