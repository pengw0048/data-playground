#!/usr/bin/env python3
"""Fail a release gate when an open P0/P1 UX defect remains.

The tracking issue itself is intentionally excluded: it describes the gate, not a defect in a
golden workflow. Every other open issue carrying ``ux`` plus ``P0`` or ``P1`` is release-blocking.
"""
...
def blockers(issues: Iterable[dict], tracking_issue: int = TRACKING_ISSUE) -> list[dict]:
    ...
    for issue in issues:
        ...
        if (issue.get("number") != tracking_issue and "pull_request" not in issue
                and "ux" in labels and labels & BLOCKING_PRIORITIES):
            result.append(issue)
    return sorted(result, key=lambda issue: int(issue["number"]))


def fetch_open_ux_issues(repo: str, token: str | None, api_url: str) -> list[dict]:
    query = urlencode({"state": "open", "labels": "ux", "per_page": 100})
...
def main() -> None:
    ...
    open_blockers = blockers(fetch_open_ux_issues(args.repo, args.token, args.api_url))
    if open_blockers:
        lines = [f"#{issue['number']} {issue['title']} ({issue['html_url']})" for issue in open_blockers]
        raise SystemExit("Release blocked by open P0/P1 UX golden-workflow defects:\n" + "\n".join(lines))
    print("No open P0/P1 UX golden-workflow defects.")
