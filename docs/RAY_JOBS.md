...
Jobs does not transport the hub's admitted exact-revision input manifest. Such a run is rejected before
attempt identity, artifact allocation, driver start, or Jobs API submission; Jobs must not reopen a
mutable source head as a substitute.
