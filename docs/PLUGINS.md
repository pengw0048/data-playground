...
| `reg.add_telemetry_sink(fn)` | run observability (export finished-run telemetry) | `fn(record: dict)` invoked once per FINISHED run with a normalized record: `canvas_id/run_id/request_id/status/rows/ms/error/output_table/placement/per_node` (`per_node` = `[{node_id, label, status, rows, ms}]`). Core ships **no** exporter (offline-first) — an OTel/StatsD/warehouse sink is a plugin; a sink that raises or times out is caught + logged, never failing the run. Retained as a compatibility seam alongside typed `MetricEvent`/`AuditEvent` sinks — see [`OBSERVABILITY.md`](OBSERVABILITY.md) and `dp_run_log`. |
| `reg.add_metric_sink(fn)` | ops metrics (OPS-01) | `fn(MetricEvent)` — low-cardinality counters/histograms/gauges. See [`OBSERVABILITY.md`](OBSERVABILITY.md). |
| `reg.add_audit_sink(fn)` | security/ops audit trail (OPS-01) | `fn(AuditEvent)` — auth, sharing, settings, job submit/cancel, …. Schemas for agent egress / secret refs / policy denial are defined for downstream issues. |
