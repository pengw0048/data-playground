...
[MCP](MCP.md), and [browser support](BROWSER_SUPPORT.md).
...
multi-tenant service. User-authored code and installed plugins run with the process permissions of the
local deployment.
...
| Trusted shared service | Possible with the documented service components, but operators are responsible for deployment hardening, data policy, backups, and topology validation. |
| Distributed execution | The optional Ray plugin has an explicitly documented support matrix and validation boundary. It is not a claim that every Ray deployment is production-ready. |
...
sinks. A plugin can be distributed separately as a private or third-party package; the core must remain
